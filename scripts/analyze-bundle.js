...
function printBundleStats(stats) {
  ...
  if (stats.chunks) {
    ...
    sortedChunks.forEach(chunk => {
      console.log(
        `  ${chunk.names.join(', ') || chunk.id}: ${formatBytes(chunk.size)}`
      );
    });
  }
...
function generateRecommendations(bundleSize, largeFiles) {
  ...
  recommendations.push({
    ...
    description:
      'Use dynamic imports, optimize images with next/image, and enable compression.',
    priority: 'low',
  });
...
function analyzeFiles() {
  ...
  if (!fs.existsSync(staticDir)) {
    console.log(
      '⚠️  Static directory not found. Make sure the project is built.'
    );
...
async function analyzeBundleSize() {
  ...
  try {
    ...
    console.log(
      '⚠️  Detailed analysis failed, continuing with basic analysis...'
    );
  }
...
function compareWithPreviousReport(currentReport) {
  ...
  try {
    const previousReport = JSON.parse(
      fs.readFileSync(path.join(BUNDLE_REPORT_DIR, reportFiles[1]), 'utf8')
    );
    ...
    console.log(
      `${diffEmoji} Total bundle size: ${currentReport.totalSizeFormatted} (${diffSign}${formatBytes(diff)}, ${diffSign}${diffPercent}%)`
    );
    ...
    if (filesDiff !== 0) {
      ...
      console.log(
        `${filesEmoji} Large files: ${currentLargeCount} (${filesDiff >= 0 ? '+' : ''}${filesDiff})`
      );
    }
  } catch (error) {
...
async function main() {
  try {
    ...
    console.log(
      '📊 Check your browser for the detailed interactive report (if available).'
    );
