...
function denormalizeCompetitions(
  normalized: NormalizedCompetitions
): Competition[] {
    ...
    (set, get) => ({
      ...
      addCompetition: competition => {
        set(state => {
          const newCompetitions = [
            ...denormalizeCompetitions(state.competitions),
            competition,
          ];
      ...
      updateCompetition: (id, updates) => {
        set(state => {
          ...
          const newCompetitions = denormalizeCompetitions(
            state.competitions
          ).map(c => (c.id === id ? updatedCompetition : c));
          const normalized = normalizeCompetitions(newCompetitions);
      ...
      getCompetitionsByPriority: priority => {
        const { competitions } = get();
        return competitions.byPriority[priority].map(
          id => competitions.byId[id]
        );
      },
...
export const competitionsSelectors = {
  ...
  getUpcomingWeekCompetitions: (state: CompetitionsState) => {
    ...
    return state
      .getCompetitionsByRange(today.toISOString().split('T')[0], nextWeekDate)
      .filter(c => c.status === 'upcoming');
  },
  ...
  getUpcomingMonthCompetitions: (state: CompetitionsState) => {
    ...
    return state
      .getCompetitionsByRange(today.toISOString().split('T')[0], nextMonthDate)
      .filter(c => c.status === 'upcoming');
  },
