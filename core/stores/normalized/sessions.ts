...
export const useSessionsStore = create<SessionsState>()(
  persist(
    (set, get) => ({
      ...
      deleteSession: id => {
        set(state => {
          const newSessions = denormalizeSessions(state.sessions).filter(
            s => s.id !== id
          );
          const normalized = normalizeSessions(newSessions);
      ...
      getTotalDistance: () => {
        ...
        return allSessions.reduce(
          (total, session) => total + session.distance_meters,
          0
        );
      },
      ...
      getAverageRPE: () => {
        ...
        const totalRPE = allSessions.reduce(
          (total, session) => total + session.rpe,
          0
        );
