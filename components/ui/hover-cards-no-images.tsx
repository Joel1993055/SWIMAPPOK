...
interface HoverCardNoImageProps {
  label: string;
  title: string;
  description: string;
...
interface HoverCardsSectionNoImageProps {
  cards: HoverCardNoImageProps[];
  className?: string;
}

export function HoverCardNoImage({ 
  label, 
  title, 
  description, 
  icon,
      ...
      <div className="relative">
        ...
        <div className="relative group-hover:-translate-y-1 transition-transform duration-500 ease-in-out h-60 flex items-center justify-center">
          ...
          <div className="group-hover:opacity-0 transition-opacity duration-500 flex items-center justify-center w-full h-full">
            {icon || (
              <div className="w-32 h-32 bg-gradient-to-br from-indigo-500/20 to-purple-600/20 rounded-2xl flex items-center justify-center">
                <div className="w-16 h-16 bg-gradient-to-br from-indigo-400 to-purple-500 rounded-xl"></div>
              </div>
            )}
          </div>
          {/* Icono hover */}
          <div className="absolute top-0 left-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center w-full h-full">
            {hoverIcon || (
              <div className="w-32 h-32 bg-gradient-to-br from-indigo-600/30 to-purple-700/30 rounded-2xl flex items-center justify-center">
                <div className="w-16 h-16 bg-gradient-to-br from-indigo-300 to-purple-400 rounded-xl"></div>
              </div>
            )}
          </div>
        </div>
...
export function HoverCardsSectionNoImage({ cards, className }: HoverCardsSectionNoImageProps) {
  return (
...
export function ExampleHoverCardsNoImage() {
  const exampleCards: HoverCardNoImageProps[] = [
    {
      label: "Entrenamiento",
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 md:px-6 py-24">
      <HoverCardsSectionNoImage cards={exampleCards} />
    </div>
  );
}
