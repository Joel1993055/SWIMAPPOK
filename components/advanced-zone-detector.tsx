      ...
      <Card className="bg-muted/50">
        ...
        <CardContent>
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm mt-2">El sistema detectará automáticamente zonas, distancias y métricas</p>
          </div>
        </CardContent>
      </Card>
    );
    ...
    <div className="space-y-4">
      ...
      <Card className="bg-muted/50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {detection.isLoading && <Loader2 className="h-4 w-4 animate-spin" />}
          </CardTitle>
          <CardDescription>
              ...
              {detection.detectedZones.map((zoneResult, index) => (
                <div key={index} className="p-4 border rounded-lg bg-background/50">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium">
                        {detection.zoneDistribution[zoneResult.zone]?.toFixed(1) || 0}%
                      </span>
                  ...
                  <div className="text-xs text-muted-foreground mb-2">
                    {detection.zoneBreakdown[zoneResult.zone]?.distance.toFixed(0) || 0}m estimados
                  </div>
      ...
      {detection.suggestions.length > 0 && (
        <Alert>
          ...
          <AlertDescription>
            <div className="space-y-2">
              <p className="font-medium">Sugerencias inteligentes para mejorar tu entrenamiento:</p>
      ...
      <Card className="bg-muted/50">
        ...
        <CardContent>
          <div className="text-sm text-muted-foreground space-y-2">
            <p>• <strong>Análisis Multi-Capa:</strong> Combina palabras clave, patrones, métricas y contexto</p>
            <p>• <strong>IA Contextual:</strong> Considera el tipo de entrenamiento y fase actual</p>
            <p>• <strong>Confianza Calculada:</strong> Cada detección incluye un nivel de confianza</p>
            <p>• <strong>Tiempo Real:</strong> Análisis automático mientras escribes</p>
          </div>
        </CardContent>
      </Card>

      {/* Advertencia si no se detectan zonas */}
      {detection.detectedZones.length === 0 && content.length > 50 && !detection.isLoading && (
        <Alert>
