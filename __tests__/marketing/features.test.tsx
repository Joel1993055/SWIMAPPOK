...
describe('Features Component', () => {
  it('renders with default title and description', () => {
    ...
    expect(
      screen.getByText(
        /Launch UI makes it easy to build an unforgettable website/
      )
    ).toBeInTheDocument();
  });
  ...
  it('has proper tab descriptions', () => {
    ...
    expect(
      screen.getByText(/Choose among 100\+ components to build a landing page/)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
    ...
    expect(
      screen.getByText(/Make design yours in no time by changing the variables/)
    ).toBeInTheDocument();
  });
