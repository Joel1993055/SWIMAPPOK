...
describe('SessionsStore Normalized', () => {
  ...
  describe('setSessions', () => {
    it('should normalize and store sessions correctly', () => {
      ...
      expect(result.current.sessions.byId['session-2']).toEqual(
        mockSessions[1]
      );
    });
    ...
    it('should index sessions by date correctly', () => {
      ...
      expect(result.current.sessions.byDate['2024-01-01']).toContain(
        'session-1'
      );
      expect(result.current.sessions.byDate['2024-01-02']).toContain(
        'session-2'
      );
    });
    ...
    it('should index sessions by competition correctly', () => {
      ...
      expect(result.current.sessions.byCompetition['comp-1']).toContain(
        'session-1'
      );
      expect(result.current.sessions.byCompetition['comp-1']).toContain(
        'session-2'
  ...
  describe('updateSession', () => {
    it('should update an existing session', () => {
      ...
      expect(result.current.sessions.byId['session-1'].name).toBe(
        'Updated Session'
      );
    });
    ...
    it('should not update non-existent session', () => {
      ...
      act(() => {
        result.current.setSessions([mockSession]);
        result.current.updateSession('non-existent', {
  ...
  describe('deleteSession', () => {
    ...
    it('should update indexes when deleting a session', () => {
      ...
      expect(result.current.sessions.byDate['2024-01-01']).not.toContain(
        'session-1'
      );
      expect(result.current.sessions.byPhase['phase-1']).not.toContain(
        'session-1'
      );
      expect(result.current.sessions.byCompetition['comp-1']).not.toContain(
        'session-1'
      );
    });
  });
  ...
  describe('selectors', () => {
    ...
    it('should get sessions by competition', () => {
      ...
      const sessionsByCompetition =
        result.current.getSessionsByCompetition('comp-1');
      expect(sessionsByCompetition).toHaveLength(2);

      const emptySessions =
        result.current.getSessionsByCompetition('non-existent');
      expect(emptySessions).toHaveLength(0);
    });
    ...
    it('should get sessions by date range', () => {
      ...
      const sessionsByRange = result.current.getSessionsByRange(
        '2024-01-01',
        '2024-01-02'
      );
      expect(sessionsByRange).toHaveLength(2);

      const emptySessions = result.current.getSessionsByRange(
        '2024-01-03',
        '2024-01-04'
      );
      expect(emptySessions).toHaveLength(0);
