  ...
  const retryImport = async (): Promise<{ default: T }> => {
    try {
      ...
      if (attemptCount < retries) {
        attemptCount++
        console.warn(`Failed to load component ${chunkName || 'unknown'}, retrying... (attempt ${attemptCount}/${retries})`)
      ...
      console.error(`Failed to load component ${chunkName || 'unknown'} after ${retries} attempts`)
      throw error
    }
  }
...
export const TeamPage = createLazyComponent(
  () => import('@/app/equipo/page'),
  { chunkName: 'team', fallback: RouteFallback }
)
...
export const LogPage = createLazyComponent(
  () => import('@/app/log/page'),
  { chunkName: 'log', fallback: RouteFallback }
)
...
export const createHoverPreloader = (routeName: keyof typeof routePreloaders) => ({
...
export const preloadHeavyComponents = () => {
  setTimeout(() => {
    ...
    preloadRoute(() => import('@/components/features/dashboard/dashboard-charts'))
    preloadRoute(() => import('@/components/features/reports/report-generator'))
  }, 5000)
}
