...
class SessionsApiService {
  ...
  async searchSessions(query: string): Promise<ApiResponse<Session[]>> {
    try {
      ...
      const { data, error } = await this.supabase
        ...
        .eq('user_id', user.id)
        .or(
          `name.ilike.%${query}%,main_set.ilike.%${query}%,notes.ilike.%${query}%`
        )
        .order('date', { ascending: false });
  ...
  > {
    try {
      ...
      const strokeDistribution = sessions.reduce(
        (acc, s) => {
      ...
      const sessionTypeDistribution = sessions.reduce(
        (acc, s) => {
      ...
      const err =
        error instanceof Error
          ? error
          : new Error('Failed to get sessions stats');
      this.captureError(err, {
        component: 'SessionsApiService',
...
export function useSessionsApi() {
  return {
    ...
    getSessionsPaginated:
      sessionsApiService.getSessionsPaginated.bind(sessionsApiService),
    searchSessions: sessionsApiService.searchSessions.bind(sessionsApiService),
    getSessionsStats:
      sessionsApiService.getSessionsStats.bind(sessionsApiService),
  };
}
