...
export const competitionsKeys = {
  ...
  list: (filters: Record<string, unknown>) => [...competitionsKeys.lists(), filters] as const,
  ...
  upcoming: (limit: number) => [...competitionsKeys.all, 'upcoming', limit] as const,
  byPriority: (priority: string) => [...competitionsKeys.all, 'priority', priority] as const,
}
...
export function useUpcomingCompetitionsQuery(limit: number = 5) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) throw new Error('Failed to fetch upcoming competitions')
      return response.json() as Promise<Competition[]>
    },
    staleTime: 2 * 60 * 1000, // 2 minutos
  })
...
export function useCompetitionsByPriorityQuery(priority: 'low' | 'medium' | 'high') {
  return useQuery({
    queryKey: competitionsKeys.byPriority(priority),
    queryFn: async () => {
      const response = await fetch(`/api/competitions/priority/${priority}`)
      if (!response.ok) throw new Error('Failed to fetch competitions by priority')
      return response.json() as Promise<Competition[]>
    },
    staleTime: 2 * 60 * 1000, // 2 minutos
  })
...
export function useCreateCompetitionMutation() {
  ...
  return useMutation({
    mutationFn: async (competitionData: Omit<Competition, 'id' | 'created_at' | 'updated_at' | 'user_id'>) => {
      const response = await fetch('/api/competitions', {
        method: 'POST',
    ...
    onMutate: async (newCompetition) => {
      ...
      const previousCompetitions = queryClient.getQueriesData({ queryKey: competitionsKeys.all })
      ...
      queryClient.setQueriesData(
        ...
        (old: Competition[] | undefined) => {
          ...
          return old.map(competition => 
            competition.id === context?.optimisticCompetition.id ? data : competition
          )
        }
      )
    ...
    onMutate: async ({ id, updates }) => {
      ...
      queryClient.setQueriesData(
        ...
        (old: Competition[] | undefined) => {
          ...
          return old.map(competition => 
            competition.id === id 
              ? { ...competition, ...updates, updated_at: new Date().toISOString() }
              : competition
          )
        }
      )
...
export function useInvalidateCompetitions() {
  ...
  return {
    ...
    invalidateUpcoming: (limit: number = 5) => {
      queryClient.invalidateQueries({ queryKey: competitionsKeys.upcoming(limit) })
    },
    invalidateByPriority: (priority: string) => {
      queryClient.invalidateQueries({ queryKey: competitionsKeys.byPriority(priority) })
    }
  }
}
