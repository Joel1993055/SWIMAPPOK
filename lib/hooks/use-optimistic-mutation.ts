  ...
  return {
    ...
    > => ({
      ...
      applyOptimisticUpdate: (queryClient, { id, updates }, context) => {
        ...
        queryClient.setQueryData(queryKeys.detail(id), (old: T | undefined) => {
          if (!old) return old;
      ...
      updateWithServerData: (queryClient, data, { id }, context) => {
        queryClient.setQueriesData(
          ...
          (old: T[] | undefined) => {
            if (!old) return old;
            return old.map(item => (item.id === id ? data : item));
          }
        );
...
export function useOptimisticCreate<T extends { id: string }>(
  mutationFn: (
    data: Omit<T, 'id' | 'created_at' | 'updated_at' | 'user_id'>
  ) => Promise<T>,
  queryKeys: {
    all: (string | number)[];
    lists: () => (string | number)[];
