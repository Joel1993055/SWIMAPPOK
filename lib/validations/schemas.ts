...
export const sessionSchema = baseEntitySchema.extend({
  ...
  stroke_type: z.enum(['freestyle', 'backstroke', 'breaststroke', 'butterfly', 'mixed']),
  session_type: z.enum(['aerobic', 'threshold', 'speed', 'technique', 'recovery']),
...
export const trainingPhaseSchema = baseEntitySchema.extend({
  name: z.string().min(1).max(200),
...
export const competitionSchema = baseEntitySchema.extend({
  ...
  target_time: z.string().regex(/^\d{1,2}:\d{2}(:\d{2})?$/).optional(),
  actual_time: z.string().regex(/^\d{1,2}:\d{2}(:\d{2})?$/).optional(),
...
export const aiCoachAdviceSchema = baseEntitySchema.extend({
  type: z.enum(['training', 'nutrition', 'recovery', 'technique', 'mental', 'equipment']),
...
export const trainingReportSchema = baseEntitySchema.extend({
...
export const sessionFiltersSchema = z.object({
  ...
  competition_id: z.string().uuid().optional(),
  stroke_type: z.enum(['freestyle', 'backstroke', 'breaststroke', 'butterfly', 'mixed']).optional(),
  session_type: z.enum(['aerobic', 'threshold', 'speed', 'technique', 'recovery']).optional(),
}).refine(
  (data) => {
...
export const sessionFormSchema = z.object({
  ...
  distance: z.number().min(0, 'Distance must be positive').max(100000, 'Distance must be less than 100km'),
  duration: z.number().min(0, 'Duration must be positive').max(1440, 'Duration must be less than 24 hours'),
